# ============================================================
# المراقب الصامت – النسخة النهائية (المصححة بالكامل)
# ============================================================
...
# ============================================================
# قارئ الدستور (محسن للملفات الكبيرة)
# ============================================================
...
class SacredReader:
    ...
    def _load_large_file(self):
        """قراءة الملف الكبير بشكل متدفق"""
        self._sacred_text = ""
        current_chunk = ""
        
        with open(self.file_path, 'r', encoding='utf-8') as f:
            while True:
                data = f.read(8192)  # 8KB كل مرة
                if not data:
                    break
                self._sacred_text += data
                current_chunk += data
                
                # كل 50,000 حرف تقريباً، نحفظ chunk
                if len(current_chunk) > 50000:
                    self.chunks.append(current_chunk)
                    current_chunk = ""
                    gc.collect()  # نحرر الذاكرة
...
# ============================================================
# العقل الحي (مع تحسين update_constitution للملفات الكبيرة)
# ============================================================
...
"""
    ...
    def update_constitution(self, new_text: str):
        """تحديث الدستور (معالجة متدفقة للملفات الكبيرة)"""
        if self.is_updating:
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل المراقب الصامت – النسخة النهائية")
    logger.info("=" * 60)
...
async def chat_page():
    ...
    # إصلاح الخطأ: استخدام format بدلاً من toLocaleString
        ...
        <div class="chat-messages" id="messages">
            <div class="message agent-message">
                <div class="message-bubble">
                    السلام عليكم. {'الدستور مرتفع (' + length_display + ' حرفاً). اسألني ما شئت.' if constitution_status == 'مرتفع' else 'يرجى رفع الدستور أولاً.'}
                </div>
            </div>
        </div>
...
async def upload_constitution(file: UploadFile = File(...)):
    ...
    # التحقق من الحجم (10 ميجابايت كحد أقصى)
