# ============================================================
# الحارس الصامت – النسخة النخبوية (مع تخزين دائم)
# ============================================================
# الميزات:
# 1. القرآن يُحمّل تلقائياً من CDN
# 2. جميع التطورات والذاكرة تُنسخ إلى GitHub تلقائياً
# 3. عند إعادة التشغيل، يستعيد آخر حالة
# 4. لا ينسى أبداً، حتى لو حذفت الخدمة
# ============================================================
...
# ============================================================
# دوال المزامنة مع GitHub (التخزين الدائم)
# ============================================================

def sync_to_github(local_path: str, repo_path: str, commit_message: str = None) -> bool:
    """يرفع ملف إلى GitHub (يُخزن بشكل دائم)"""
    if not GITHUB_TOKEN:
        logger.warning("⚠️ GITHUB_TOKEN غير موجود، لن يتم الرفع")
        return False
    
    if not os.path.exists(local_path):
        logger.warning(f"⚠️ الملف {local_path} غير موجود، لن يتم رفعه")
        return False
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ تم تحديث {repo_path} في GitHub")
        except:
            # إنشاء ملف جديد
            repo.create_file(repo_path, commit_message, content, branch=GITHUB_BRANCH)
            logger.info(f"✅ تم إنشاء {repo_path} في GitHub")
        
        return True
    except Exception as e:
        logger.error(f"❌ فشل رفع {repo_path} إلى GitHub: {e}")
        return False

def sync_from_github(local_path: str, repo_path: str) -> bool:
    """يحمل ملف من GitHub (يستعيد الذاكرة)"""
    if not GITHUB_TOKEN:
        return False
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ تم استعادة {repo_path} من GitHub")
            return True
        except Exception as e:
            logger.info(f"📁 الملف {repo_path} غير موجود في GitHub (سيتم إنشاؤه لاحقاً)")
            return False
    except Exception as e:
        logger.error(f"❌ فشل تحميل {repo_path} من GitHub: {e}")
        return False

def backup_agent_state(event: str = "manual"):
    """نسخ احتياطي لحالة الوكيل بالكامل إلى GitHub"""
    ...
    for local_path, repo_path in files_to_backup:
        if os.path.exists(local_path):
            if sync_to_github(local_path, repo_path, f"Backup: {event} - {datetime.now().isoformat()}"):
                success_count += 1
    
    logger.info(f"✅ تم رفع {success_count}/{len(files_to_backup)} ملف إلى GitHub")
    
    # أيضاً رفع الكتب (إذا وجدت)
...
def restore_agent_state():
    """استعادة حالة الوكيل من GitHub (آخر نسخة محفوظة)"""
...
class DatabaseManager:
    ...
    def get_backup_data(self) -> dict:
        ...
        data = {
            "decisions": self.fetch_all("SELECT * FROM decisions"),
            "evolutions": self.fetch_all("SELECT * FROM evolutions"),
            "constitutional_memory": self.fetch_all("SELECT * FROM constitutional_memory"),
            "chat_history": self.fetch_all("SELECT * FROM chat_history"),
        }
        return data
...
"""
        ...
        try:
            ...
            if self._test_brain(new_brain):
                ...
                # نسخ احتياطي بعد التطور الناجح
                backup_agent_state("evolution_success")
...
app = FastAPI(title="الحارس الصامت - النسخة النخبوية")
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل الحارس الصامت – النسخة النخبوية (مع تخزين دائم)")
    logger.info("=" * 60)
    
    # ============================================================
    # استعادة الذاكرة من GitHub (آخر نسخة)
    # ============================================================
    restore_agent_state()
    ...
    # 1. حاول من قاعدة البيانات
    row = db.fetch_one("SELECT text FROM constitution_cache WHERE id = 1")
    if row and row[0]:
        constitution_text = row[0]
        logger.info(f"✅ تم تحميل الدستور من قاعدة البيانات: {len(constitution_text):,} حرفاً")
    
    # 2. إذا لم يكن موجوداً، حمّل من الرابط
    if not constitution_text:
        logger.info(f"🌐 جاري تحميل القرآن من الرابط: {QURAN_URL}")
        try:
            ...
            logger.info(f"✅ تم تحميل القرآن من الرابط: {len(constitution_text):,} حرفاً، {len(quran_data)} سورة")
            
            db.execute(
                "INSERT OR REPLACE INTO constitution_cache (id, text, updated_at) VALUES (1, ?, ?)",
                (constitution_text, datetime.now().isoformat())
            )
            logger.info(f"💾 تم حفظ الدستور في قاعدة البيانات")
        except Exception as e:
    ...
    if constitution_text:
        ...
        if not os.path.exists(FLAG_FILE):
            ...
            logger.info("✅ تم إنشاء FLAG_FILE")
    
    if not constitution_text:
        logger.error("❌ فشل تحميل الدستور!")
...
<head>
    ...
    <title>الحارس الصامت - نخبوي</title>
    <style>
...
<body>
    <div class="chat-container">
        <div class="chat-header">
            <h1>🤖 الحارس الصامت</h1>
            <p>وكيل دستوري حي | يتطور ذاتياً | ذاكرة دائمة | مزاج: {mood_name}</p>
        </div>
        <div class="book-area">
            📚 رفع كتاب (مدخل إضافي):
        ...
        <div class="chat-messages" id="messages">
            <div class="message agent-message">
                <div class="message-bubble">
                    السلام عليكم. {'الدستور مرتفع (' + length_display + ' حرفاً). مزاجي: ' + mood_name + '. الكتاب الحالي: ' + current_book + '. اسألني ما شئت.' if constitution_status == 'مرتفع' else 'جاري تحميل الدستور...'}
                </div>
            </div>
        </div>
...
async def chat(req: ChatRequest):
    ...
    if not mind.sacred_text:
        return JSONResponse({"response": "⚠️ جاري تحميل الدستور. حاول مرة أخرى خلال دقيقة."})
...
async def upload_constitution(file: UploadFile = File(...)):
    return JSONResponse({
        "error": "الدستور يتم تحميله تلقائياً من الرابط. لا يمكن تغييره."
    }, status_code=403)
...
async def constitution_excerpt():
    ...
    if mind and mind.sacred_text:
        return JSONResponse({
            ...
            "source": "Quran JSON (loaded from CDN + backed up)"
        })
...
async def reset_design():
    ...
    try:
        ...
        backup_agent_state("design_reset")
...
@app.post("/force-flag")
async def force_flag():
    try:
        with open(FLAG_FILE, 'w') as f:
            f.write(datetime.now().isoformat())
        logger.info("✅ تم إنشاء FLAG_FILE عبر نقطة الطوارئ")
        return JSONResponse({"status": "success", "message": "FLAG_FILE created"})
    except Exception as e:
        logger.error(f"فشل إنشاء FLAG_FILE: {e}")
        return JSONResponse({"error": str(e)}, status_code=500)

@app.post("/manual-backup")
async def manual_backup():
    """نسخ احتياطي يدوي لحالة الوكيل"""
