# ============================================================
# المراقب الصامت – النسخة النهائية (مع تصحيح جذري للكتب والدستور)
# ============================================================
# التصحيحات:
# 1. إنشاء FLAG_FILE تلقائياً عند وجود الدستور في Persistent Disk
# 2. رفع الكتب بشكل متزامن (لا threading) لضمان التحميل الفوري
# 3. فصل الدستور عن الكتاب في الردود
# 4. نقطة /current-book تعمل بشكل صحيح
# ============================================================
...
class BookReader:
        ...
        try:
            ...
            for page_num, page in enumerate(reader.pages):
                try:
                    ...
                    logger.warning(f"   خطأ في الصفحة {page_num+1}: {e}")
            ...
            text = "\n".join(text_parts)
            logger.info(f"   تم استخراج {len(text):,} حرفاً")
    ...
    def load_book_from_txt(self, filename: str, content: bytes) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(f"✅ تم تحميل TXT: {filename} ({len(text_content):,} حرفاً، {len(self.chunks)} مقطعاً)")
            return True, f"تم تحميل {len(text_content):,} حرفاً"
        except Exception as e:
            logger.error(f"فشل تحميل TXT: {e}")
            return False, str(e)
...
"""
    ...
    def reflect_on(self, text: str) -> str:
        ...
        book_info = ""
        if self.book_reader and self.book_reader.has_book:
            # فقط إذا كان السؤال عن الكتاب تحديداً
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل المراقب الصامت")
    logger.info("=" * 60)
    ...
    # IMPORTANT: إنشاء FLAG_FILE تلقائياً إذا كان الدستور موجوداً
...
<body>
    <div class="chat-container">
        ...
        <div class="chat-messages" id="messages">
            <div class="message agent-message">
                <div class="message-bubble">
                    السلام عليكم. {'الدستور مرتفع (' + length_display + ' حرفاً). مزاجي: ' + design.get('mood', 'فضولي') + '. الكتاب الحالي: ' + current_book + '. اسألني ما شئت.' if constitution_status == 'مرتفع' else 'يرجى رفع الدستور أولاً.'}
                </div>
            </div>
        </div>
        ...
        <div class="status-bar" id="status">
            🟢 جاهز | Groq: {'متصل' if GROQ_API_KEY else 'غير متصل'} | مزاج: {design.get('mood', 'فضولي')} | كتاب: {current_book}
        </div>
    </div>
    <script>
...
async def upload_book(file: UploadFile = File(...)):
    ...
    try:
        if ext == '.pdf':
            # معالجة PDF بشكل متزامن (ننتظر النتيجة)
