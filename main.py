...
# النسخة النخبوية النهائية v5.1
# ============================================================
# التعديلات النخبوية في هذه النسخة:
# 1. إضافة مسار المشروع إلى PYTHONPATH لضمان عمل الاستيرادات
# 2. استخدام asyncio.Lock للمتغيرات العامة (أمان في البيئة السحابية)
# 3. دالة get_mind_safe آمنة للاستخدام عبر الطلبات
# 4. تحسين إدارة الأخطاء في بدء التشغيل
# 5. توافق كامل مع Render وبيئة Docker
# ============================================================
...
def background_loop():
    ...
    logger.info("🟢 بدء حلقة الخلفية المتكاملة v5.1")
    time.sleep(15)
...
async def lifespan(app: FastAPI):
    ...
    global _db, _book_reader, _background_thread
    
    logger.info("=" * 70)
    logger.info("🚀 تشغيل الحارس الصامت - النسخة النخبوية v5.1")
    logger.info("=" * 70)
    ...
    # تهيئة الذاكرة
    memory = ConstitutionalMemory(_db)

    # تحميل الدستور
    constitution_text = ""
    row = _db.fetch_one("SELECT text FROM constitution_cache WHERE id = 1")
    if row and row[0]:
    ...
    logger.info("=" * 70)
    logger.info("✨ الحارس الصامت v5.1 يعمل الآن")
...
# ============================================================
# دالة إضافية للمساعدة في التصحيح
# ============================================================
...
async def debug_import_errors():
    ...
    modules_to_test = [
        ...
        "github.backup", "agent.living_mind", "agent.book_reader",
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        host="0.0.0.0",
        port=port,
        reload=False,  # في الإنتاج، reload يجب أن يكون False
        workers=1  # عدد العمال (للمشاريع الصغيرة)
    )
