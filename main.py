# ============================================================
# المراقب الصامت – النسخة النهائية المحسنة للملفات الكبيرة
# تتضمن:
# 1. معالجة متدفقة (streaming) للملفات الكبيرة
# 2. تحميل الدستور في الخلفية
# 3. تقسيم تدريجي دون استهلاك ذاكرة زائد
# 4. تحسين timeout و memory usage
# ============================================================
...
# ============================================================
# قارئ الدستور المقدس (محسن للملفات الكبيرة)
# ============================================================
...
class SacredReader:
    ...
    def _load(self):
        """تحميل النص مع معالجة تدريجية للملفات الكبيرة"""
        if not self.file_path.exists():
            self._sacred_text = ""
            return
        
        # للملفات الكبيرة (أكثر من 1 ميجابايت)، نقرأها بشكل متدفق
    ...
    def _load_large_file(self):
        """قراءة الملف الكبير بشكل متدفق وتقسيمه فوراً"""
        self._sacred_text = ""
        current_chunk = ""
        ...
        with open(self.file_path, 'r', encoding='utf-8') as f:
            while True:
                chunk = f.read(4096)  # نقرأ 4KB كل مرة
                if not chunk:
                    break
                self._sacred_text += chunk
                current_chunk += chunk
                
                if len(current_chunk) > chunk_size_chars * 5:
                    self.chunks.append(current_chunk[:chunk_size_chars * 5])
                    current_chunk = ""
            
            if current_chunk:
                self.chunks.append(current_chunk[:chunk_size_chars * 5])
        
        if not self.chunks and self._sacred_text:
            self._chunk_text()
    
    def _chunk_text(self, chunk_size: int = 500):
        """تقسيم النص العادي (للملفات الصغيرة)"""
    ...
    def append_text(self, additional_text: str):
        """إضافة نص إلى الدستور الحالي (للملفات الكبيرة)"""
        if not self._sacred_text:
            self._sacred_text = additional_text
        else:
            self._sacred_text += additional_text
        
        words = additional_text.split()
        chunk_size = 500
        for i in range(0, len(words), chunk_size):
            chunk = " ".join(words[i:i+chunk_size])
            self.chunks.append(chunk)
...
# ============================================================
# العقل الحي (محسن للملفات الكبيرة)
# ============================================================
...
"""
    ...
    def update_constitution(self, new_text: str):
        """تحديث الدستور (مع معالجة تدريجية للملفات الكبيرة)"""
        self._log("INFO", f"بدء تحديث الدستور: {len(new_text):,} حرفاً")
        
        chunk_size = 50000
        total_chunks = (len(new_text) + chunk_size - 1) // chunk_size
        
        for i in range(0, len(new_text), chunk_size):
            partial_text = new_text[i:i+chunk_size]
            self.sacred_text += partial_text
            
            if i == 0:
                self.reader.reload(partial_text)
            else:
                self.reader.append_text(partial_text)
            
            progress = int((i + chunk_size) / len(new_text) * 100)
            if progress % 20 == 0:
                self._log("INFO", f"تحديث الدستور: {progress}%")
            
            time.sleep(0.1)
        
        self.sacred_text = new_text
        self._log("INFO", f"✅ تم تحديث الدستور بالكامل: {len(self.sacred_text):,} حرفاً")
...
app = FastAPI(title="المراقب الصامت - النسخة المحسنة")
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل المراقب الصامت – النسخة المحسنة للملفات الكبيرة")
    logger.info("=" * 60)
    ...
    try:
        ...
        mind.memory = memory
        logger.info("✅ الوكيل جاهز (بدون دستور)")
    except Exception as e:
        logger.error(f"❌ فشل إنشاء الوكيل: {e}")
        return
    
    if os.path.exists(CONSTITUTION_FILE):
        logger.info("📖 يوجد دستور محفوظ، جاري تحميله في الخلفية...")
        
        def load_constitution_background():
            try:
                with open(CONSTITUTION_FILE, 'r', encoding='utf-8') as f:
                    text = f.read()
                mind.update_constitution(text)
                logger.info(f"✅ تم تحميل الدستور المحفوظ: {len(text):,} حرفاً")
            except Exception as e:
                logger.error(f"❌ فشل تحميل الدستور المحفوظ: {e}")
        
        thread = threading.Thread(target=load_constitution_background)
        thread.daemon = True
        thread.start()
...
<body>
    <div class="chat-container">
        <div class="chat-header">
            ...
            <p>وكيل دستوري حي | يتطور ذاتياً | نسخة محسنة للملفات الكبيرة</p>
        </div>
        <div class="upload-area">
            📁 رفع دستورك (يدعم الملفات حتى 10 ميجابايت):
    ...
    <script>
        async function uploadConstitution() {{
            ...
            if (file.size > 10 * 1024 * 1024) {{ alert('الملف كبير جداً (حد أقصى 10 ميجابايت)'); return; }}
            const formData = new FormData();
            formData.append('file', file);
            document.getElementById('uploadStatus').innerText = 'جاري الرفع (قد يستغرق دقيقة)...';
            try {{
                const response = await fetch('/upload', {{ method: 'POST', body: formData }});
                const data = await response.json();
                if (data.status === 'success') {{
                    document.getElementById('uploadStatus').innerHTML = '✅ تم رفع: ' + data.filename + ' (' + data.characters.toLocaleString() + ' حرفاً)';
                    document.getElementById('status').innerHTML = '🟢 جاهز | يتطور ذاتياً';
                    setTimeout(() => location.reload(), 2000);
                }} else {{
                    document.getElementById('uploadStatus').innerHTML = '❌ فشل الرفع: ' + (data.error || 'خطأ غير معروف');
                }}
            }} catch(e) {{
...
async def upload_constitution(file: UploadFile = File(...)):
    ...
    if len(content) > 10 * 1024 * 1024:
        return JSONResponse({"error": "الملف كبير جداً (حد أقصى 10 ميجابايت)"}, status_code=400)
    ...
    try:
        ...
        if mind:
            def update_in_background():
                try:
                    ...
                    logger.error(f"فشل تحديث الدستور في الوكيل: {e}")
        ...
        return JSONResponse({
            ...
            "characters": len(text),
            "message": "تم الرفع بنجاح. قد يستغرق تحميل الدستور دقيقة."
        })
