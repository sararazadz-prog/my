...
"""
    ...
    def _self_evaluate(self, decision: dict, result: dict, gate_passed: bool) -> float:
        """تقييم ذاتي – 5 معايير (من Colab)"""
        action = decision.get("action", "silence")
        score = 0.0
        criteria = 0
        
        # معيار 1: اجتياز البوابة الدستورية
        if gate_passed:
            score += 0.3
            criteria += 1
        else:
