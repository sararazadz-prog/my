# ============================================================
# المراقب الصامت – النسخة النهائية (مع تصحيح رفع الكتب)
# ============================================================
# التصحيحات:
# 1. رفع الكتب حتى 10 ميجابايت مع معالجة متدفقة
# 2. رد فوري للمستخدم دون انتظار
...
class BookReader:
    ...
    def _load_large_book(self, book_file: Path):
        """قراءة الكتاب الكبير بشكل متدفق"""
        self.current_book_text = ""
        current_chunk = ""
    ...
    def load_book(self, filename: str, content: bytes):
        """تحميل كتاب جديد - معالجة متدفقة"""
        file_path = self.books_dir / filename
        logger.info(f"📚 بدء تحميل الكتاب: {filename} ({len(content):,} بايت)")
        
        # حفظ الملف كاملاً (للمعالجة المستقبلية)
        with open(file_path, 'wb') as f:
            f.write(content)
        
        # قراءة المحتوى ومعالجته
        try:
            # معالجة متدفقة للنص
            ...
            logger.info(f"✅ تم تحميل كتاب جديد: {filename} ({len(self.current_book_text):,} حرفاً، {len(self.chunks)} مقطعاً)")
            return True
        except Exception as e:
            logger.error(f"فشل معالجة الكتاب: {e}", exc_info=True)
            return False
    
    def load_book_pdf(self, filename: str, pdf_path: str):
        """تحميل كتاب PDF - معالجة متدفقة"""
        try:
            ...
            logger.info(f"📚 بدء تحميل PDF: {filename} ({total_pages} صفحة)")
            ...
            logger.info(f"✅ تم تحميل PDF: {txt_filename} ({len(text):,} حرفاً)")
            return True
        except Exception as e:
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل المراقب الصامت – النسخة النهائية (كتب مصححة)")
    logger.info("=" * 60)
    ...
    <script>
        ...
        async function uploadBook() {{
            ...
            try {{
                ...
                document.getElementById('bookStatus').innerHTML = '❌ خطأ في الاتصال: ' + e.message;
            }}
        }}
...
async def upload_book(file: UploadFile = File(...)):
    ...
    # رد فوري للمستخدم أن العملية بدأت
    temp_id = str(uuid.uuid4())[:8]
        ...
        if ext == '.pdf':
            ...
            def process_pdf():
                try:
                    ...
                    if success:
                        logger.info(f"✅ تم معالجة PDF بنجاح: {file.filename}")
                    else:
            ...
            return JSONResponse({
                "status": "success",
                "filename": file.filename,
                "message": "جاري معالجة PDF في الخلفية. قد يستغرق دقيقة."
            })
        else:
            # TXT - معالجة مباشرة
            try:
                ...
                if success:
                    logger.info(f"✅ تم رفع كتاب TXT: {file.filename} ({len(text_content):,} حرفاً)")
                    return JSONResponse({
                        "status": "success",
                        "filename": file.filename,
