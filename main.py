# ============================================================
# الحارس الصامت – النسخة النهائية (مع مسار الملف المطلق)
# ============================================================
# الدستور في المستودع (constitution.txt)
# لا حاجة إلى رفعه عبر الواجهة
# B2 اختياري للنسخ الاحتياطي (لا يؤثر إن لم يُضبط)
# ============================================================
...
# Backblaze B2 (اختياري – للنسخ الاحتياطي فقط)
B2_KEY_ID = os.environ.get("B2_KEY_ID", "")
B2_APPLICATION_KEY = os.environ.get("B2_APPLICATION_KEY", "")
B2_BUCKET_NAME = os.environ.get("B2_BUCKET_NAME", "constitution")
B2_ENDPOINT = os.environ.get("B2_ENDPOINT", "https://s3.us-east-005.backblazeb2.com")

os.makedirs(BOOKS_DIR, exist_ok=True)

# ============================================================
# دوال Backblaze B2 (اختيارية – لا تؤثر إن لم تُضبط)
# ============================================================

def get_b2_client():
    """إنشاء عميل B2 فقط إذا كانت المتغيرات موجودة"""
...
def upload_constitution_to_b2(content: str) -> bool:
    """رفع إلى B2 إذا كان متاحاً (نسخة احتياطية)"""
    ...
    try:
        client.put_object(
            ...
            Key='constitution.txt',
            Body=content.encode('utf-8'),
            ContentType='text/plain'
        )
        logger.info("✅ تم رفع الدستور إلى B2 (نسخة احتياطية)")
        return True
    except Exception as e:
        logger.warning(f"⚠️ فشل رفع النسخة الاحتياطية إلى B2: {e}")
        return False

def download_constitution_from_b2() -> Optional[str]:
    """تحميل من B2 إذا كان متاحاً (نسخة احتياطية)"""
    client = get_b2_client()
    if not client:
        return None
    try:
        response = client.get_object(Bucket=B2_BUCKET_NAME, Key='constitution.txt')
        content = response['Body'].read().decode('utf-8')
        logger.info("✅ تم تحميل الدستور من B2 (نسخة احتياطية)")
        return content
    except:
        return None
...
app = FastAPI(title="الحارس الصامت")
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل الحارس الصامت – الدستور من المستودع")
    logger.info("=" * 60)
    ...
    # ============================================================
    # تحميل الدستور (الأولوية: ملف محلي في المستودع ← B2 احتياطي)
    # باستخدام Path(__file__).parent لضمان العثور على الملف
    # ============================================================
    constitution_text = ""
    
    # 1. الأولوية القصوى: الملف المحلي في المستودع (باستخدام المسار المطلق)
    current_dir = Path(__file__).parent
    local_constitution_file = current_dir / "constitution.txt"
    
    logger.info(f"🔍 البحث عن الدستور في: {local_constitution_file}")
    logger.info(f"📁 الملفات الموجودة في {current_dir}: {os.listdir(current_dir)}")
    
    if local_constitution_file.exists():
        try:
            with open(local_constitution_file, 'r', encoding='utf-8') as f:
                constitution_text = f.read()
            logger.info(f"✅ تم تحميل الدستور من الملف المحلي: {len(constitution_text):,} حرفاً")
            
            # نسخ إلى Persistent Disk للاستخدام السريع
            with open(CONSTITUTION_FILE, 'w', encoding='utf-8') as f:
                f.write(constitution_text)
        except Exception as e:
            logger.error(f"❌ فشل تحميل الدستور من الملف المحلي: {e}")
    else:
        logger.warning(f"⚠️ الملف {local_constitution_file} غير موجود")
    
    # 2. إذا لم ينجح الملف المحلي، حاول B2 كنسخة احتياطية
    if not constitution_text:
        constitution_text = download_constitution_from_b2()
        if constitution_text:
            with open(local_constitution_file, 'w', encoding='utf-8') as f:
                f.write(constitution_text)
            logger.info("✅ تم استعادة الدستور من B2 إلى الملف المحلي")
    
    # 3. إذا لم ينجح أي شيء
    if not constitution_text:
        logger.error("❌ لا يوجد دستور! تأكد من وجود constitution.txt في المستودع")
    
    # إنشاء FLAG_FILE إذا كان الدستور موجوداً
    if constitution_text and not os.path.exists(FLAG_FILE):
        with open(FLAG_FILE, 'w') as f:
            f.write(datetime.now().isoformat())
        logger.info("✅ تم إنشاء FLAG_FILE تلقائياً")
...
async def chat_page():
    # الدستور الآن في المستودع → لا يحتاج إلى رفع
...
async def chat(req: ChatRequest):
    ...
    if not mind.sacred_text:
        return JSONResponse({"response": "⚠️ الدستور غير متوفر. تأكد من وجود constitution.txt في المستودع."})
...
async def upload_constitution(file: UploadFile = File(...)):
    # الدستور الآن في المستودع، يمنع رفعه عبر الواجهة
    return JSONResponse({
        "error": "الدستور موجود بالفعل في المستودع. لا يمكن تغييره عبر الواجهة."
    }, status_code=403)
...
async def constitution_excerpt():
    ...
    if mind and mind.sacred_text:
        return JSONResponse({
            ...
            "source": "constitution.txt (repository)",
            "b2_configured": bool(B2_KEY_ID and B2_APPLICATION_KEY)
        })
