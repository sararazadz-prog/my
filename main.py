# ============================================================
# الحارس الصامت – المرحلة 1: ثبات الدستور مع Backblaze B2
# ============================================================
# الإضافات الجديدة:
# 1. تحميل الدستور من Backblaze B2 عند بدء التشغيل
# 2. رفع الدستور إلى B2 عند الرفع عبر الواجهة
# 3. استعادة الدستور تلقائياً بعد أي إعادة تشغيل
# ============================================================
...
# متغيرات Backblaze B2
B2_KEY_ID = os.environ.get("B2_KEY_ID", "")
B2_APPLICATION_KEY = os.environ.get("B2_APPLICATION_KEY", "")
B2_BUCKET_NAME = os.environ.get("B2_BUCKET_NAME", "constitution-storage")
...
def get_b2_client():
    """إنشاء عميل Backblaze B2 (متوافق مع S3 API)"""
    if not B2_KEY_ID or not B2_APPLICATION_KEY:
        logger.warning("⚠️ B2 credentials not configured")
        return None
    ...
    try:
        ...
        logger.info(f"✅ B2 client created for bucket: {B2_BUCKET_NAME}")
        return client
    except Exception as e:
        logger.error(f"❌ فشل إنشاء عميل B2: {e}")
        return None

def upload_constitution_to_b2(content: str) -> bool:
    """رفع الدستور إلى Backblaze B2"""
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ B2 bucket exists: {B2_BUCKET_NAME}")
        except:
            # إنشاء bucket إذا لم يكن موجوداً
            client.create_bucket(Bucket=B2_BUCKET_NAME)
            logger.info(f"✅ تم إنشاء bucket: {B2_BUCKET_NAME}")
        ...
        logger.info(f"✅ تم رفع الدستور إلى B2: {len(content):,} حرفاً")
        return True
    except Exception as e:
        logger.error(f"❌ فشل رفع الدستور إلى B2: {e}")
        return False

def download_constitution_from_b2() -> Optional[str]:
    """تحميل الدستور من Backblaze B2"""
    client = get_b2_client()
    if not client:
        return None
    ...
    try:
        ...
        logger.info(f"✅ تم تحميل الدستور من B2: {len(content):,} حرفاً")
        return content
    except Exception as e:
        logger.warning(f"⚠️ لم يتم العثور على الدستور في B2: {e}")
        return None
...
async def startup_event():
    ...
    logger.info("=" * 60)
    logger.info("🚀 تشغيل الحارس الصامت – المرحلة 1: ثبات الدستور مع Backblaze B2")
    logger.info("=" * 60)
    ...
    # ============================================================
    # تحميل الدستور (الأولوية: B2 ← ملف محلي ← جديد)
    # ============================================================
    constitution_text = ""
    
    # 1. حاول تحميل الدستور من Backblaze B2 (الأساسي)
    constitution_text = download_constitution_from_b2()
    
    if constitution_text:
        logger.info(f"✅ تم تحميل الدستور من Backblaze B2: {len(constitution_text):,} حرفاً")
        # حفظ في الملف المحلي للاستخدام السريع
        with open(CONSTITUTION_FILE, 'w', encoding='utf-8') as f:
        ...
        logger.info("✅ تم إنشاء FLAG_FILE (الدستور من B2)")
    
    else:
        # 2. إذا لم ينجح B2، حاول من الملف المحلي (كاحتياطي)
    ...
    # إنشاء FLAG_FILE إذا كان الدستور موجوداً والعلامة مفقودة
    if constitution_text and not os.path.exists(FLAG_FILE):
        with open(FLAG_FILE, 'w') as f:
            f.write(datetime.now().isoformat())
        logger.info("✅ تم إنشاء FLAG_FILE تلقائياً (الدستور موجود)")
    
    # تأكيد وجود FLAG_FILE في السجلات
    if os.path.exists(FLAG_FILE):
        logger.info(f"📍 FLAG_FILE موجود: {FLAG_FILE}")
    else:
        logger.warning(f"⚠️ FLAG_FILE غير موجود: {FLAG_FILE}")
...
async def upload_constitution(file: UploadFile = File(...)):
    ...
    try:
        ...
        # حفظ في Persistent Disk
        with open(CONSTITUTION_FILE, 'w', encoding='utf-8') as f:
            f.write(text)
        
        # رفع إلى Backblaze B2
        upload_constitution_to_b2(text)
...
async def force_flag():
    """نقطة طوارئ لإنشاء FLAG_FILE يدوياً"""
    try:
        with open(FLAG_FILE, 'w') as f:
            f.write(datetime.now().isoformat())
