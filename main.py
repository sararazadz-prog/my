...
# 2. إخفاء زر رفع الدستور نهائياً بعد أول رفع (باستخدام FLAG_FILE)
# 3. الواجهة المتغيرة حسب مزاج الوكيل (ألوان، أشكال)
# 4. قراءة الكتب كمدخلات إضافية (PDF و TXT مع معالجة متدفقة)
# 5. نقطة /current-book للتحقق من الكتاب المفتوح
# ============================================================
...
class BookReader:
    ...
    def _load_current_book(self):
        ...
        try:
            ...
            logger.info(f"📚 تحميل الكتاب الحالي: {self.current_book_name} ({file_size:,} بايت)")
            
            if file_size > 1024 * 1024:
                self._load_large_book(book_file)
            else:
            ...
            logger.error(f"فشل تحميل الكتاب {self.current_book_name}: {e}", exc_info=True)
            self.current_book_text = ""
            self.current_book_name = ""
            self.chunks = []
    
    def _load_large_book(self, book_file: Path):
        self.current_book_text = ""
        current_chunk = ""
    ...
    def load_book(self, filename: str, content: bytes) -> bool:
        file_path = self.books_dir / filename
        logger.info(f"📚 بدء تحميل كتاب TXT: {filename} ({len(content):,} بايت)")
        ...
        try:
            ...
            logger.info(f"✅ تم تحميل كتاب TXT: {filename} ({len(text_content):,} حرفاً، {len(self.chunks)} مقطعاً)")
            return True
        except Exception as e:
            logger.error(f"فشل تحميل كتاب TXT {filename}: {e}", exc_info=True)
            return False
    
    def load_book_pdf(self, filename: str, pdf_path: str) -> bool:
        logger.info(f"📚 بدء تحميل PDF: {filename}")
        ...
        try:
            ...
            logger.info(f"   PDF: {total_pages} صفحة")
            ...
            for page_num, page in enumerate(reader.pages):
                try:
                    ...
                    if page_text and len(page_text.strip()) > 10:
                        ...
                        logger.warning(f"   الصفحة {page_num+1} لا تحتوي على نص واضح")
                except Exception as e:
                    logger.warning(f"   خطأ في استخراج الصفحة {page_num+1}: {e}")
                
                if page_num % 10 == 0:
                    gc.collect()
            
            if not text_parts:
                logger.error("❌ لم يتم استخراج أي نص من PDF")
                return False
            
            text = "\n".join(text_parts)
            logger.info(f"   تم استخراج {len(text):,} حرفاً من PDF")
            ...
            logger.info(f"✅ تم تحميل PDF وتحويله إلى نص: {txt_filename} ({len(text):,} حرفاً، {len(self.chunks)} مقطعاً)")
            return True
            
        except ImportError:
            logger.error("❌ مكتبة pypdf غير متوفرة")
            return False
        except Exception as e:
            logger.error(f"❌ فشل معالجة PDF: {e}", exc_info=True)
            return False
...
async def startup_event():
    ...
    if os.path.exists(CONSTITUTION_FILE):
        try:
            ...
            # IMPORTANT: إنشاء ملف العلامة إذا كان الدستور موجوداً والعلامة مفقودة
...
async def chat_page():
    # IMPORTANT: إخفاء زر رفع الدستور بناءً على وجود FLAG_FILE فقط
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* زر رفع الدستور يختفي نهائياً بعد أول رفع */
        .upload-area {{
            background: #f0f0f0;
            padding: 10px;
            text-align: center;
...
async def upload_constitution(file: UploadFile = File(...)):
    ...
    # منع رفع الدستور إذا كان موجوداً مسبقاً
    ...
    try:
        ...
        # إنشاء ملف العلامة بعد الرفع الناجح
        with open(FLAG_FILE, 'w') as f:
            f.write(datetime.now().isoformat())
...
async def upload_book(file: UploadFile = File(...)):
    ...
    try:
        if ext == '.pdf':
            def process_pdf():
                try:
                    success = book_reader.load_book_pdf(file.filename, tmp_path)
                    if success:
                        logger.info(f"✅ تمت معالجة PDF بنجاح: {file.filename}")
                        logger.info(f"   الكتاب الحالي: {book_reader.get_current_book()}")
                        logger.info(f"   عدد المقاطع: {len(book_reader.chunks)}")
                    else:
                        logger.error(f"❌ فشل معالجة PDF: {file.filename}")
                except Exception as e:
                    logger.error(f"خطأ في معالجة PDF: {e}", exc_info=True)
                finally:
                    if os.path.exists(tmp_path):
                        os.unlink(tmp_path)
            ...
            return JSONResponse({
                "status": "success",
                "filename": file.filename,
                "message": "جاري معالجة PDF في الخلفية. قد يستغرق دقيقة. اسألني عن محتواه بعد قليل."
            })
        else:
            try:
                success = book_reader.load_book(file.filename, content)
                if success:
                    logger.info(f"✅ تم رفع كتاب TXT: {file.filename}")
                    return JSONResponse({
                        "status": "success",
                        "filename": file.filename,
                        "characters": len(book_reader.current_book_text),
                        "chunks": len(book_reader.chunks),
                        "message": f"تم الرفع. الكتاب الآن مفتوح: {file.filename}"
                    })
                else:
                    return JSONResponse({"error": "فشل تحميل الكتاب"}, status_code=500)
            except Exception as e:
                logger.error(f"فشل معالجة TXT: {e}", exc_info=True)
                return JSONResponse({"error": str(e)[:200]}, status_code=500)
            finally:
                if os.path.exists(tmp_path):
                    os.unlink(tmp_path)
